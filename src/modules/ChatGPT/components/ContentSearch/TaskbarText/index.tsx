...
const TaskbarText = forwardRef(
  ({ content, messages, fetchData }: TaskbarTextProps, ref) => {
    //
    const {
      app: { current },
      dispatch,
    ...
    const handleRegenerate = async (type: "chatgpt" | "user") => {
      if (!current) return;
    ...
    return (
      <div className="flex items-center gap-2 my-3">
        {messages.list.length > 1 && (
          <ul className="flex items-center gap-3 mr-2">
            <li
        ...
        {!messages.isLoading &&
          messages.list[messages.index].type === "chatgpt" && (
            <>
              <Tooltip
                handleClick={() => {
              ...
              {current?.messages &&
                (current?.messages || [])
                  ...
                  <Tooltip
                    handleClick={() => handleRegenerate("chatgpt")}
                    title="Regenerate"
                    position="bottom"
                  >
